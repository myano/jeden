...
class jenny(irc.Bot): 
    ...
    def wrapped(self, origin, text, match): 
        class jennyWrapper(object): 
            def __init__(self, jenny): 
                self.bot = jenny
    ...
    def call(self, func, origin, jenny, input): 
        try: 
            func(jenny, input)
        except Exception, e: 
            self.error(origin)
            ...
            for regexp, funcs in items: 
                for func in funcs: 
                    ...
                    if match: 
                        ...
                        jenny = self.wrapped(origin, text, match)
                        input = self.input(origin, text, bytes, match, event, args)
                        if func.thread: 
                            targs = (func, origin, jenny, input)
                            t = threading.Thread(target=self.call, args=targs)
                            t.start()
                        else: self.call(func, origin, jenny, input)
