...
def u(jenny, input): 
   ...
   arg = input.bytes[3:]
   # jenny.msg('#inamidst', '%r' % arg)
   if not arg: 
      return jenny.reply('You gave me zero length input.')
   elif not arg.strip(' '): 
      if len(arg) > 1: return jenny.reply('%s SPACEs (U+0020)' % len(arg))
      return jenny.reply('1 SPACE (U+0020)')
   ...
   if printable: 
      ...
      if extended: 
         ...
         for i, result in enumerate(results): 
            if (i < 2) or ((i == 2) and (len(results) < 4)): 
               ...
               jenny.say(result + ' [...]')
         if not results: 
            jenny.reply('Sorry, no results')
      else: 
         # look up a codepoint freely
         result = codepoint_simple(arg)
         if result is not None: 
            jenny.say(result)
         else: jenny.reply("Sorry, no results for %r." % arg)
   else: 
      ...
      # look up more than three podecoints
         ...
         jenny.reply(' '.join('U+%04X' % ord(c) for c in text))
      else: jenny.reply('Sorry, your input is too long!')
u.commands = ['u']
u.example = '.u 203D'

def bytes(jenny, input): 
   """Show the input as pretty printed bytes."""
   b = input.bytes
   jenny.reply('%r' % b[b.find(' ') + 1:])
