...
def tr(jenny, context): 
   ...
   if input != output: 
      ...
      jenny.reply(msg)
   else: jenny.reply('Language guessing failed, so try suggesting one!')

tr.rule = ('$nick', ur'(?:([a-z]{2}) +)?(?:([a-z]{2}) +)?["“](.+?)["”]\? *$')
tr.example = '$nickname: "mon chien"? or $nickname: fr "mon chien"?'
tr.priority = 'low'

def mangle(jenny, input): 
   ...
   jenny.reply(phrase or 'ERRORS SRY')
