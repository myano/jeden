...
def yi(jenny, input): 
   ...
   if extraraels == 4: 
      return jenny.say('Yes! PARTAI!')
   else: jenny.say('Not yet...')
yi.commands = ['yi']
yi.priority = 'low'

def tock(jenny, input): 
   """Shows the time from the USNO's atomic clock."""
   u = urllib.urlopen('http://tycho.usno.navy.mil/cgi-bin/timer.pl')
   info = u.info()
   u.close()
   jenny.say('"' + info['Date'] + '" - tycho.usno.navy.mil')
tock.commands = ['tock']
tock.priority = 'high'
...
def npl(jenny, input): 
   ...
   if data: 
      ...
      jenny.say(result + ' - ntp1.npl.co.uk')
   else: jenny.say('No data received, sorry')
npl.commands = ['npl']
npl.priority = 'high'
